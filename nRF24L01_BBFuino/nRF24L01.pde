...
#define TX_PLOAD_WIDTH  1  //  1 unsigned chars TX payload
...
void loop() 
{
 //Start TX mode  
    ...
    if((key = keypad.getKey()) != NO_KEY)
    {
      ...
      lcd.setCursor(0,1);
      lcd.print("Mode    ");
      tx_buf[0] = key;                                         // store the data to an array
    ...
    delay(50);
    RX_Mode();                                               // change back to RX mode
}
  else
  {
    ...
    if(status&RX_DR)                                                 // if receive data ready (TX_DS) interrupt
    {
      ...
      SPI_RW_Reg(FLUSH_RX,0);                                        // clear RX_FIFO
          ...
          lcd.print("Receive");
          lcd.setCursor(0, 1);
          lcd.print(rx_buf[0]);                                      // display the data received on LCD
     }
    SPI_RW_Reg(WRITE_REG+STATUS,status);                             // clear RX_DR or TX_DS or MAX_RT interrupt flag
    delay(50);
    TX_Mode();                                                       // change to TX mode
  }
}
